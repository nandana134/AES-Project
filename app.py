...
@app.route('/')
def home():
    # Always pass essay and grade explicitly
    return render_template('index.html', essay='', grade=None)
...
def predict():
    if request.method == 'POST':
        ...
        if not essay_text.strip():  # Check for empty input
            ...
            return render_template('index.html', essay=essay_text, grade=None)
        try:
            ...
            return render_template('index.html', grade=round(float(pred[0][0]), 2), essay=essay_text)
